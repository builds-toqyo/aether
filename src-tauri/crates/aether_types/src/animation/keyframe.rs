...
//! 
//! This module provides the core keyframe data structures for
//! animation system including time-based keyframes and value storage.
...
impl<T> Keyframe<T> {
    ...
    /// Create a keyframe with custom interpolation
    ...
    /// Create a keyframe with custom interpolation and easing
    pub fn with_easing(
        time: f64,
        value: T,
        interpolation: crate::animation::interpolation::InterpolationMethod,
...
impl<T: fmt::Display> fmt::Display for Keyframe<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Keyframe(t: {:.3}, v: {}, interp: {:?}, ease: {:?})", 
...
mod tests {
    ...
    fn test_keyframe_validation() {
        ...
        let invalid_keyframes = vec![
            Keyframe::new(1.0, 1.0),
            Keyframe::new(1.0, 2.0), // Duplicate time
        ];
