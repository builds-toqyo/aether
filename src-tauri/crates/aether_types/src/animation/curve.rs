//! Animation curve data structures
//! 
//! This module provides animation curve definitions for different
//! interpolation methods including linear, bezier, and ease curves.
...
impl BezierControlPoint {
    ...
    /// Create control point with handles
...
impl AnimationCurve {
            ...
            CurveType::Bezier | CurveType::Custom => {
                ...
                for i in 1..self.control_points.len() {
                    if self.control_points[i].position.0 < self.control_points[i - 1].position.0 {
                        return Err("Control points must be ordered by x coordinate".to_string());
                    }
                }
            }
            _ => {
                if !self.control_points.is_empty() {
                    return Err(format!("{} curve does not support control points", self.curve_type.name()));
                }
            }
        }
    ...
    pub fn description(&self) -> String {
        match self.curve_type {
            CurveType::Linear => "Linear interpolation between keyframes".to_string(),
            CurveType::Bezier => format!("Bezier curve with {} control points", self.control_points.len()),
            CurveType::EaseIn => "Ease-in acceleration curve".to_string(),
            CurveType::EaseOut => "Ease-out deceleration curve".to_string(),
            CurveType::EaseInOut => "Ease-in-out smooth curve".to_string(),
            CurveType::Step => "Step function with no interpolation".to_string(),
...
/// Curve builder for constructing complex curves
pub struct CurveBuilder {
    curve: AnimationCurve,
}

impl CurveBuilder {
    /// Create new curve builder
    pub fn new() -> Self {
        Self {
    ...
    /// Add control point with handles
...
mod tests {
    ...
    fn test_ease_curves() {
        ...
        // Test that ease curves produce different results
        let t = 0.5;
