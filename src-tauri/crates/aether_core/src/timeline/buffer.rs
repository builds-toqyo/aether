...
/// Pre-roll buffer for smooth scrubbing start
...
impl PreRollBuffer {
    ...
    /// Initialize buffer for a starting position
    ...
    pub async fn get_frame(&self, position: TimelinePosition) -> Result<Option<PreviewFrame>> {
        ...
        // Search for frame at or near the requested position
    ...
    /// Pre-fill buffer with frames around target position
    ...
    /// Check if buffer is ready for position
...
impl Default for BufferConfig {
    fn default() -> Self {
        Self {
            ...
            auto_refill: true,
            refill_threshold: 0.5, // Refill when less than 50% full
        }
    }
}
...
mod tests {
    ...
    async fn test_pre_roll_buffer_tolerance() {
        ...
        // Add frame at position 100
        let frame = create_test_frame(position);
        buffer.add_frame(position, frame).await.unwrap();
        
        // Should find frame at position 102 (within tolerance)
        let nearby_pos = TimelinePosition::from_frame(102);
        let cached = buffer.get_frame(nearby_pos).await.unwrap();
        assert!(cached.is_some());
        
        // Should not find frame at position 110 (outside tolerance)
    ...
    async fn test_pre_roll_buffer_pre_fill() {
        ...
        assert!(pre_filled > 0);
        assert!(pre_filled <= 7); // 3 before + 1 center + 3 after
        
        // Check that center position is buffered
    ...
    async fn test_pre_roll_buffer_size_limit() {
        ...
        // Add 5 frames (should only keep 3 most recent)
        for i in 0..5 {
