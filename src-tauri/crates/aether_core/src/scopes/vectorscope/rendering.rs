//! Vectorscope rendering and visualization
//! 
//! This module handles the rendering of vectorscope data into images,
//! including intensity visualization and color gradients.
...
/// Renderer for vectorscope data visualization
pub struct VectorscopeRenderer {
    intensity_colors: Vec<Rgb<u8>>,
}

impl VectorscopeRenderer {
    /// Create new vectorscope renderer
    pub fn new() -> Self {
        let mut intensity_colors = Vec::new();
        
        // Precompute intensity colors for performance
        for i in 0..1000 {
    ...
    /// Draw intensity grid with color gradients
    ...
    fn get_intensity_color(&self, intensity: u16) -> Rgb<u8> {
        if intensity < self.intensity_colors.len() as u16 {
            ...
            // Use highest intensity color for values beyond precomputed range
