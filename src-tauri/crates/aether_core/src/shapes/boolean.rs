...
//! 
//! This module provides boolean operations (union, subtract, intersect)
//! for combining and manipulating vector shapes.
    ...
    pub fn operate_paths(
        ...
        if !path_a.closed || !path_b.closed {
            return BooleanResult::failure(
                operation,
                "Both paths must be closed for boolean operations".to_string(),
            );
        }
...
impl AdvancedBoolean {
    ...
    pub fn multiple_operations(
        ...
        for (i, &shape) in shapes.iter().enumerate().skip(1) {
            ...
            if !result.success {
                return Err(format!("Boolean operation failed at step {}: {:?}", i, result.error));
            }
...
impl BooleanUtils {
    ...
    /// Get intersection area between two shapes
    ...
    /// Simplify a shape by removing redundant vertices
    ...
    /// Simplify a path by removing redundant vertices
...
// Import PathSegment for use in this module
...
mod tests {
    ...
    fn test_convex_hull() {
        ...
        // Should contain the 4 corner points but not the interior point
        assert!(hull.len() >= 4);
    ...
    fn test_simplify_shape() {
        ...
        assert!((simplified.area() - rect.area()).abs() < 100.0); // Allow some error
    }
}
