...
/// Transform operations for 2D image transformations
pub struct TransformOperations {
    params: TransformParams,
}

impl TransformOperations {
    /// Create new transform operations
    ...
    fn perform_transform(&self, texture_id: Uuid, matrix: &TransformMatrix) -> Uuid {
        ...
        // Upload transformed texture to GPU
    ...
    /// Apply transformation to pixel data
    ...
    /// Transform a point using current parameters
    pub fn transform_point(&self, x: f32, y: f32) -> (f32, f32) {
        let matrix = self.get_matrix();
        matrix.transform_point(x, y)
    }
    
    /// Transform a vector using current parameters
