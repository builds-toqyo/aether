...
impl MergeNode {
    ...
    fn perform_real_blend(&self, texture_id1: Uuid, texture_id2: Uuid) -> Uuid {
        ...
        log::debug!("Performing real blend operation: {} + {} with mode={:?}", 
        ...
        let blended_data = match self.blend_mode.as_str() {
            "normal" => self.blend_normal(&data1, &data2, width, height, channels),
            "multiply" => self.blend_multiply(&data1, &data2, width, height, channels),
            "screen" => self.blend_screen(&data1, &data2, width, height, channels),
            "overlay" => self.blend_overlay(&data1, &data2, width, height, channels),
            "add" => self.blend_add(&data1, &data2, width, height, channels),
            "subtract" => self.blend_subtract(&data1, &data2, width, height, channels),
            _ => self.blend_normal(&data1, &data2, width, height, channels), // Default to normal
        };
        
        // Upload blended result to GPU
        let blended_texture_id = self.upload_blended_texture(&blended_data, width, height, channels);
        
        log::debug!("Blend operation completed: {}x{} texture with ID {}", 
    ...
    fn upload_blended_texture(&self, data: &[u8], width: usize, height: usize, channels: usize) -> Uuid {
        ...
        log::debug!("Uploading blended texture to GPU: {}x{} ({} channels)", 
        ...
        log::debug!("Blended texture uploaded to GPU with ID: {}", texture_id);
