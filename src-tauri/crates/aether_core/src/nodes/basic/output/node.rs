...
mod tests {
    ...
    fn test_quality_settings_validation() {
        ...
        // Test clamping
        settings.set_jpeg_quality(150); // Should clamp to 100
        assert_eq!(settings.jpeg_quality, 100);
        
        settings.set_jpeg_quality(0); // Should clamp to 1
        assert_eq!(settings.jpeg_quality, 1);
        
        settings.set_png_compression(15); // Should clamp to 9
        assert_eq!(settings.png_compression, 9);
        
        settings.set_png_compression(-1); // Should clamp to 0
        assert_eq!(settings.png_compression, 0);
        
        settings.set_video_bitrate(2000); // Should clamp to 1000
        assert_eq!(settings.video_bitrate, 1000);
        
        settings.set_video_bitrate(0); // Should clamp to 1
        assert_eq!(settings.video_bitrate, 1);
        
        settings.set_color_depth(12); // Should default to 8
        assert_eq!(settings.color_depth, 8);
        
        settings.set_color_depth(64); // Should default to 8
        assert_eq!(settings.color_depth, 8);
        
        settings.set_color_depth(16); // Should stay 16
        assert_eq!(settings.color_depth, 16);
        
        settings.set_color_depth(32); // Should stay 32
    ...
    fn test_is_active() {
        ...
        // Should not be active with Raw format
        assert!(!output_node.is_active());
        
        // Should be active with other formats
        output_node.set_output_format(OutputFormat::PngSequence);
        assert!(output_node.is_active());
        ...
        // Should not be active again with Raw
