...
/// Core merge node - combines multiple inputs
#[derive(Debug)]
...
impl CoreMergeNode {
    /// Create a new core merge node
    pub fn new(node: Node) -> Self {
        Self { node }
    }
    
    /// Get blend mode from node parameters
    ...
    /// Get opacity from node parameters
    ...
    /// Apply blend operation between multiple inputs
    fn apply_blend(&self, inputs: &[ParameterValue]) -> ParameterValue {
        ...
        // - Apply blend operation based on mode and opacity
        // - Handle different blend modes (normal, multiply, screen, etc.)
        // - Return blended result
        
        // For now, return the first non-None input
        for input in inputs {
...
mod tests {
    ...
    fn test_apply_blend() {
        ...
        // Test with multiple valid inputs (should return first)
    ...
    fn test_node_executor_interface() {
        ...
        // Test can execute without inputs
        let context = ExecutionContext {
            frame: 1,
            inputs: std::collections::HashMap::new(),
            outputs: std::collections::HashMap::new(),
